...
export default class MyBufferedProcess {
  /*
  ...
  */
  ...
  start () {
    ...
    this.started = true
    this.exec(this.command, this.options)
  ...
  */
  ...
  exec (command, options) {
    try {
      this.process = ChildProcess.exec(command, options)
    } catch (execError) {
      process.nextTick(() => this.handleError(execError))
    }
  }
